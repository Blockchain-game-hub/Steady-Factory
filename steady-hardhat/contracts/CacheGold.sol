...
contract CacheGold is IERC20, Ownable {
    ...
    function addBackedTokens(uint256 value) external onlyOwner returns (bool) {
        ...
        require(
            lockedGrams >= totalCirculation().add(value),
        ...
        returns (bool)
    {
        ...
        require(
            newFeeAddress != _unbackedTreasury,
     ...
     */
    function setRedeemAddress(address newRedeemAddress)
        external
        onlyOwner
        returns (bool)
    {
        ...
        require(
            newRedeemAddress != _unbackedTreasury,
     ...
     */
    function setBackedAddress(address newBackedAddress)
        external
        onlyOwner
        returns (bool)
    {
        ...
        require(
            newBackedAddress != _unbackedTreasury,
     ...
     */
    function setUnbackedAddress(address newUnbackedAddress)
        external
        onlyOwner
        returns (bool)
    {
        ...
        require(
            newUnbackedAddress != _backedTreasury,
            "Cannot set unbacked treasury to backed treasury"
        );
        require(
            newUnbackedAddress != _feeAddress,
            "Cannot set unbacked treasury to fee address "
        );
        require(
            newUnbackedAddress != _redeemAddress,
            "Cannot set unbacked treasury to fee address "
        );
     ...
     */
    function setOracleAddress(address oracleAddress)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function calcStorageFee(address account) public view returns (uint256) {
        ...
        if (daysInactive >= INACTIVE_THRESHOLD_DAYS) {
            ...
            daysSinceStoragePaid = daysSinceStoragePaid.sub(
                daysInactive.sub(INACTIVE_THRESHOLD_DAYS)
            );
        }
    ...
    function calcSendAllBalance(address account) public view returns (uint256) {
        ...
        uint256 balanceAfterStorage = _balances[account].sub(
        ...
        uint256 divisor = TOKEN.add(
            _transferFeeBasisPoints.mul(BASIS_POINTS_MULTIPLIER)
        );
        uint256 sendAllAmount = balanceAfterStorage.mul(TOKEN).div(divisor).add(
            1
        );

        // Calc transfer fee on send all amount
        uint256 transFee = sendAllAmount.mul(_transferFeeBasisPoints).div(
        ...
        returns (uint256)
    {
        uint256 fee = balance
            .mul(TOKEN)
            .mul(daysSinceStoragePaid)
            .div(YEAR)
            .div(STORAGE_FEE_DENOMINATOR);
        if (fee > balance) {
            return balance;
        }
        return fee;
    ...
    function _transfer(
        ...
        if (from != to) {
            ...
            _balances[_feeAddress] = _balances[_feeAddress].add(allFeeFrom).add(
                storageFeeTo
            );
        } else {
    ...
    function _mint(uint256 value) internal returns (bool) {
        require(
            _totalSupply.add(value) <= SUPPLY_CAP,
            "Call would exceed supply cap"
        );
        require(
            _balances[_unbackedTreasury] == 0,
            "The unbacked treasury balance is not 0"
        );
    ...
    function _calcInactiveFeePerYear(uint256 snapshotBalance)
        ...
        returns (uint256)
    {
        uint256 inactiveFeePerYear = snapshotBalance.mul(TOKEN).div(
